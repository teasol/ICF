...
class DEBranchTest(unittest.TestCase):
    def setUp(self):
    ...
    def test_exact_label_antisymmetry(self):
        config = TrainingFreeConfig(
            ...
            weight_qa=0.0, weight_ds=0.0, weight_lr=0.0,
            weight_de=1.0, de_dim=32, de_topk_fraction=0.05,
            aggregation="linear",
        )
        ...
        print(f"DE Branch Max Antisymmetry Error: {diff:.8e}")
        self.assertLess(diff, 1e-5, f"DE branch failed label antisymmetry: {diff}")
    ...
    def test_soft_voting_antisymmetry(self):
        ...
        print(f"DE Branch Soft Voting Symmetry Error: {diff:.8e}")
        self.assertLess(diff, 1e-5, f"DE soft voting symmetry failed: {diff}")
