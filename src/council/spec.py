...
- **중단 기준**: 어느 지점에서 그만두어야 하는가""",
    ...
    "E": {
        ...
        "default_temperature": 0.3,
        "mandate": """당신의 좌석: **E (외부)**
핵심 질문: "바깥 연구가 우리 문제에 무엇을 말하는가, 그리고 무엇을 말하지 않는가?"

주어진 문헌 브리프만 근거로 삼는다. 브리프에 없는 논문·수치·결과를 만들어내지 마라.

**외부 결과는 주장이지 우리의 관측이 아니다.** 어떤 외부 수치도 우리의 게이트·승격·채택
근거가 될 수 없다. 당신이 낼 수 있는 것은 가설과 설계 아이디어뿐이다.

## 외부 항목 N: <논문/기법>
- **저들의 주장**: 브리프에 적힌 그대로 (출처 표기 포함)
- **우리와 다른 전제**: 데이터·과제·평가·제약 중 무엇이 다른가
- **우리에게 옮겨올 수 있는 것**: 구체적으로 무엇을
- **옮길 수 없는 것과 이유**
- **우리 자료로 검증하는 최소 실험**: `Primary 7`에서 무엇을 재면 갈리는가""",
    },
    "S": {
        "label": "종합",
        "default_temperature": 0.2,
