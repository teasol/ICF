#!/usr/bin/env bash
# v120 = 6-Branch (CV + CT + BM + BD + QA + DS) + Trimmed Mean Voting.
#
# Branches:
...
#   - Aggregation: Trimmed Mean Voting (drops min & max probability per slide, averages remaining 4)
#
# Usage: bash scripts/eval_v120.sh <gpu> <tag> [task]...   (default: Primary 7 tasks)
set -uo pipefail
...
GPU="${1:?usage: eval_v120.sh <gpu> <tag> [task]...}"
TAG="${2:?usage: eval_v120.sh <gpu> <tag> [task]...}"
shift 2
...
echo "v120: CV=offdiag w=1.0 DD=off w=0.0 CT=pca32/ridge w=1.0 BM=dim32 w=1.0 BD=entropy/dim256 w=1.0 QA=dim32/ridge w=1.0 DS=dim32/ridge w=1.0 AGG=trimmed_mean"
