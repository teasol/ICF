"""Train/evaluate Hard orthogonal v76 with learned ridge calibration on GPU 0-3."""
...
def main() -> None:
    ...
    print(f"=== TRAIN START hard_ridge_calibration GPUs={GPUS}", flush=True)
    ...
    workers = [
        subprocess.Popen(
            ["bash", "scripts/eval_seal_tasks.sh", str(gpu), str(checkpoint),
             CONFIG, "v76_hard_ridge_calibration_best", *tasks], cwd=ROOT,
        )
    ...
    print("=== EVAL END hard_ridge_calibration", flush=True)
