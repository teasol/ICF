...
#
# Usage: bash scripts/eval_v39_ridge_ablation.sh [gpu]
set -uo pipefail
...
for arm in baseline no_abundance no_covariance; do
  # Newest run directory for this arm, then its lowest-val_ce checkpoint.
  run_dir=$(ls -dt checkpoints/*/v39_${arm} 2>/dev/null | head -1)
  if [[ -z "$run_dir" ]]; then
    echo "!! no checkpoint dir for v39_${arm} -- skipping"
    continue
  fi
  ...
  out="predictions/pathobench_bc_therapy_er_status_v39_${arm}_official50_bf16.pt"
  log="logs/official50/er_status_v39_${arm}_official50_bf16.log"
  echo "=== START v39_${arm} $(date)"
  echo "    ckpt: $ckpt"
  CUDA_VISIBLE_DEVICES="$GPU" "$PY" scripts/test_pathobench.py \
      --checkpoint "$ckpt" \
      --config "configs/train_v39_${arm}_1536.yaml" \
      --official-folds "$TASK_DIR" \
      --features "$FEATURES" \
      --input-dim 1536 \
      --precision bf16-mixed \
      --output "$out" > "$log" 2>&1
  echo "=== END v39_${arm} rc=$? $(date)"
