"""Train the 8-GPU v76 combo arm, then immediately run SEAL evaluation."""
...
CONFIG = "configs/archive/v69_v76_relation/train_v76_combo_classsep_rare_noise_8gpu_1536.yaml"
CHECKPOINT_DIR = ROOT / "checkpoints/20260812_v76_combo/classsep_rare_noise_8gpu"
LOG_DIR = ROOT / "logs/20260812_v76_combo"
TAG = "v76_combo_classsep_rare_noise_8gpu_best"
TASK_GROUPS = (
...
def main() -> None:
    ...
    print("=== TRAIN START classsep+rare+noise (8 GPU)", flush=True)
    with (LOG_DIR / "train.out").open("w") as output:
        subprocess.run(
            [
                PYTHON,
                "scripts/train.py",
    ...
    print("=== EVAL END classsep+rare+noise", flush=True)
