#!/usr/bin/env bash
# Evaluate the v38 ridge-ablation set (docs SS65) on the default task
...
#
# Usage: bash scripts/eval_v38_ridge_ablation.sh [gpu]
set -uo pipefail
...
for arm in control global abundance covariance; do
  # Newest run directory for this arm, then its lowest-val_ce checkpoint.
  run_dir=$(ls -dt checkpoints/*/v38_${arm} 2>/dev/null | head -1)
  if [[ -z "$run_dir" ]]; then
    echo "!! no checkpoint dir for v38_${arm} -- skipping"
    continue
  fi
  ...
  out="predictions/pathobench_bc_therapy_er_status_v38_${arm}_official50_bf16.pt"
  log="logs/official50/er_status_v38_${arm}_official50_bf16.log"
  echo "=== START v38_${arm} $(date)"
  echo "    ckpt: $ckpt"
  CUDA_VISIBLE_DEVICES="$GPU" "$PY" scripts/test_pathobench.py \
      --checkpoint "$ckpt" \
      --config "configs/train_v38_ridge_ablation_${arm}_1536.yaml" \
      --official-folds "$TASK_DIR" \
      --features "$FEATURES" \
      --input-dim 1536 \
      --precision bf16-mixed \
      --output "$out" > "$log" 2>&1
  echo "=== END v38_${arm} rc=$? $(date)"
