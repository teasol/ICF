#!/usr/bin/env bash
# v114 + mean/top-k CONCATENATED abundance (SS189-2). Deterministic, 0-param
# diagnostic: instead of replacing the mean abundance with top-k pooling (as in
# eval_v114_topk.sh), each token's K-dimensional descriptor becomes
# [mean; top-k mean] = 2K coordinates, so the ridge readout can use both jointly.
# Everything else is bit-identical to v114; the CT readout stays `ridge`.
#
# Usage: bash scripts/eval_v114_cattopk.sh <gpu> <tag> [task]...
...
GPU="${1:?usage: eval_v114_cattopk.sh <gpu> <tag> [task]...}"
TAG="${2:?usage: eval_v114_cattopk.sh <gpu> <tag> [task]...}"
shift 2
...
export ICF_CT_ABUNDANCE_POOLING=mean+topk
...
echo "v114+cattopk: pooling=mean+topk fraction=${ICF_CT_ABUNDANCE_TOPK_FRACTION} min=${ICF_CT_ABUNDANCE_TOPK_MIN} (rest = v114)"
