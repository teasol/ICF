#!/usr/bin/env bash
# Non-linear Kernel Ridge Regression (KRR) evaluation runner.
#
# Branches:
#   - CV: offdiag (w=1.0)
#   - DD: OFF (w=0.0)
#   - CT: PCA-32 K256 soft abundance (w=1.0, readout=kernel_ridge)
#   - BM: PCA-32 bag mean (w=1.0)
#   - BD: Spectral Entropy ordered-typicality (w=1.0)
#   - QA: PCA-32 Quantile Evidence (w=1.0)
#   - DS: PCA-32 Salience Denoised Bag Mean (w=1.0)
#   - Aggregation: Trimmed Mean Voting
#   - Kernel: RBF (ICF_KRR_KERNEL=rbf)
#
# Usage: bash scripts/eval_krr.sh <gpu> <tag> [task]...   (default: Primary 7 tasks)
set -uo pipefail
...
GPU="${1:?usage: eval_krr.sh <gpu> <tag> [task]...}"
TAG="${2:?usage: eval_krr.sh <gpu> <tag> [task]...}"
shift 2
...
# Non-linear Kernel Settings
export ICF_KRR_KERNEL="${ICF_KRR_KERNEL:-rbf}"

echo "KRR Run ($ICF_KRR_KERNEL): CV=1.0 DD=0.0 CT=1.0 BM=1.0 BD=1.0 QA=1.0 DS=1.0 AGG=trimmed_mean KERNEL=$ICF_KRR_KERNEL"
