...
4i-0. ⚠️⚠️ **데이터 분포 축을 닫는다 — arm 13개, 양성 0개 (§125-4)**. §115(에피소드 모양)·§123
   (cell 값 분포) 두 진단 사이클 모두 "격차는 정확히 실측됐고 닫아도 오르지 않는다"로 끝났다.
   **이 축에서 새 arm을 설계하지 말 것.** 항목 5(병목이 표현이 아닐 가능성)를 먼저 검토할 것.
