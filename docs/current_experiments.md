...
현재 Hard 학습은 B200 1장에서 episode batch 4, gradient accumulation 2(effective 8 episodes/update), BF16 mixed이며, 50~100 bags에서 query 5~12개를 빼 context 38~95개를 봅니다. 최대 1,500 cells/bag의 보수적 FP32 벤치에서도 총 172 bags는 80.8GB, 220 bags는 103.4GB peak로 통과하여 context 160은 batch 4를 유지할 수 있습니다. 따라서 raw-cell audit 전에 기존 checkpoint의 짧은 40~160-context fine-tuning 신호를 확인하고, 효과가 있으면 동일 update/학습-bag 예산의 정식 재학습 비교로 승격합니다. 실제 ICI context가 약 69이므로 평가는 40/80/160을 모두 유지하며, 40~80에서도 좋아져야 학습 개선으로 인정합니다.
