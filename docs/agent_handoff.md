...
> **게이트 ② 정보량 (§218 신설)** — 게이트 ①을 통과해도 **단독 AUROC가 Primary 7 전 과제에서 0.5를 넘지 못하면 기각**한다. `BS`는 직교했으나 3개 과제에서 우연 이하여서 기각되었다 — **"직교하나 무정보"** 범주는 실재한다. 이 게이트는 라벨을 쓰지만 **후보당 사전 선언된 단일 통계**이므로, 앙상블 구성을 탐색해 최댓값을 고르는 §214식 절차와 구분된다.
