...
`current_status.md` §108–§118. **v89**(bag 축)는 미판정(Δ−0.0048, t=−1.39, 1/4, §115-7 —
bag·cell 두 축이 함께 움직여 소진으로 쓰지 말 것), **v90**(class_prior 축, nhn-SMC)은 통계
게이트는 근소 미달(Δ−0.0053, t≈−2.23, **4/4**)이었지만 **§118에서 사용자가 종합적 판단으로
기각**했다 — task 10개 중 하락 7개가 baseline 고성능(천장 근접) 구간에 몰리고 상승 3개는 전부
저신호(0.5 근접) 구간이었던 비대칭이 근거다. **⚠️ §118로 판정 프로토콜 자체가 바뀌었다** —
§107-3 게이트는 계속 계산·보고하되 **최종 승격/기각은 게이트 통과 여부가 아니라 task 10개 전부를
baseline 성능대별로 살펴본 사용자의 종합 판단**이다(agent_handoff §판정계약 ⑬). 새 arm 보고 시
task 10개 전부를 baseline과 나란히 표로 보여줄 것 — 요약 통계만 보고하지 말 것.
