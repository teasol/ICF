...
보편 규범 §1의 실패 양식을 적용한다. Era 6에 대한 사용자의 문제 제기는
연구 방향 없는 수치 최적화이며, 선택 편향은 별도로 다룬다.
...
**양식 B — 연구 방향 없이 주어진 조합의 수치만 최대화** (Era 6에 대한 사용자 지적)
무엇이 한계인지, 어떤 가능성을 찾아볼지, 왜 이 후보들을 살피는지 정하지 않은 채
주어진 조합 안의 점수만 높이는 데 머물렀다는 지적이다. 방향을 찾는 탐색 자체가 빠진 문제를
사후 선택 편향으로 축소하지 않는다. 범위·예산을 정하고 정직하게 보고하거나 독립 평가를
통과해도, 그것만으로 다음 접근의 근거가 축적되는 것은 아니다.

**양식 C — 선택 결과를 확증으로 포장** (평가·보고의 별도 문제)
...
(보편 규범 §2.1). **탐색에는 기전 가설이 없어도 된다.** 무엇을 발견하려는지, 왜 이 범위와
후보를 살피는지, 결과가 후속 방향에 무엇을 알려줄지는 밝힌다. 범위·예산·선택 규칙은
이 질문에 더해 기록한다. 방향이 미정이면 방향을 찾기 위한 탐색을 설계한다.
연구 방향 없는 수치 최적화와 탐색을 확증으로 포장하는 문제를 각각 점검한다.
**창의적 구조 탐색(보편 규범 §2.3)에서는 필수 제약과 구현 관습을 구분하고, 불확실한 접근에도
최소 구현·프로브 예산과 중단 조건을 배정해 검증 기회를 보장한다.** 아이디어 생성과 심사를 분리하며,
초기 평가는 점수가 아니라 작동 가능성과 배울 내용을 평가하고, 조기 탈락 시 단순 보류와 실증적 반박을 구분한다.
...
- **승격의 개선량 기준은 Primary 7 평균 AUROC의 절대 증가량 +0.003 이상(= +0.3%p)**이다.
  `Δ_macro = (1/7) Σ_task [(1/50) Σ_fold (AUC_candidate − AUC_baseline)] ≥ 0.003`.
  **95% 신뢰구간 하한에 +0.3%p를 요구하는 기준이 아니다.** 상대 증가율 0.3%도 아니다.
- **불확실성의 역할**: 과제 군집 95% 신뢰구간(CI)·표준오차(SE)와 과제별 악화는 투명하게 별도로 보고한다.
...
- SD·SE·CI·MDE·`δ_min`을 하나로 혼용하지 않는다(보편 규범 §4.1). **MDE는 검출 가능한 크기,
  `δ_min`은 채택할 가치가 있는 개선량 크기(+0.3%p)**이며 서로 다르다. MDE를 성공 컷오프로 쓰지 않는다.
...
판정 설계의 정본은 `PROJECT.md` §4이며, 결정 근거는 `D-018`·`D-019`·`D-021`·`D-041`이다.
...
연구 질문을 “점수를 높이고 가장 높은 후보를 채택한다”로만 채우지 않습니다. 해결할 한계나
찾아볼 가능성, 후보 범위를 고른 이유, 결과가 다음 접근에 줄 근거를 설명합니다. 방향이
불명확하면 먼저 오류·한계·가능한 접근을 조사하는 탐색을 설계합니다.
...
목표 연결 검사를 적용한다(`D-023`). **착수할 후보는 [`research_directions.md` §0](research_directions.md)의
큐에 있어야 한다** — 큐에 없는 것을 즉흥으로 착수하지 않고, 새 후보는 우선순위와 함께 큐에 먼저 넣는다.
큐에 있다는 것은 예산 승인이나 채택을 뜻하지 않는다. 진행 상태·후속 행동은
...
`미정`으로 두어도 되지만, 무엇을 발견하려는지와 후보 범위의 선정 이유를 밝히고
범위·예산·선택 규칙을 적는다.**
...
# 2. 회귀 스위트 (153 tests, ~35s, CPU) — nexgem에서는 sbatch로 node1~5에, s1에서는 직접
...
- **RU 단위로 진행한다**: 착수 전 `scripts/docs/ru.py open`, 종료 시 `close` (§1).
- **축을 닫을 때는 경계를 함께 적는다**: `기각 기전`·`경계 안`·`경계 밖`·`재개 조건` 네 필드가 모두 필요하다 ([`closed_axes.md`](closed_axes.md)).
- **절 번호는 재사용하지 않는다** (과거 `history/archive.md`에 §199·§200이 중복 사용된 바 있음).
