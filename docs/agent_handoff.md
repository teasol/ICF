...
> ⚠️ **[§215 실측 정정] 이 브랜치들은 상보적(complementary)이지 않다.** 5-branch 앙상블의 유효 랭크는 **2.26 / 5 (명목의 45%)** 이며, `BM`·`QA`·`DS`는 모두 동일한 top-32 PCA 부분공간의 1차 모멘트로서 상호 상관 r = 0.57~0.93 (셋의 유효 랭크 **1.29 / 3**) — 사실상 하나의 신호다. 나머지와 직교하는 브랜치는 **`BD` 뿐**(|r| = 0.01~0.16)이다. **[§226 정정 — §215 시점 사실]** 이후 §218 `SH`(max |r| = 0.418)·§220 `SHJ`(max |r| = 0.227)가 형상 계열로 채택되었다. 현 시점 형상 계열은 **`BD`·`SH`·`SHJ` 3개**이며, 신규 후보의 게이트 ① 직교성은 이 3개를 모두 포함해 측정한다 (`branch_screen.py --adopted m_sh,m_shj`가 기본값). 단 `BRANCHES`(공식 5-branch 비교 기준)는 불변식 3에 따라 변경하지 않는다.
> 그 결과 Trimmed Mean은 독립 신호를 나르는 `CV`(슬라이드의 83.6%)와 `BD`(62.1%)를 우선적으로 절사하고, 중복 블록의 중앙값에 수렴한다.
> **[§226 정정 — 이 기전을 신규 직교 브랜치로 확장하지 말 것]** 위 절사율은 **기존 5-branch에서 관측된 현상**이다(5-branch 균등 기대 40%). §218 §4가 확장을 실측 반증했다 — 7-branch 구성에서 신규 후보 `BS` 22.3% / `SH` 22.3%로 **균등 기대 28.6%보다 오히려 덜 절사되었고**, archive.md는 "'신규 직교 브랜치가 절사된다'는 설명은 성립하지 않는다 (측정 전 예상과 반대였다)"고 기록한다. §218이 내린 결론은 형상 계열의 병목이 절사가 아니라 **정보량**이라는 것이다. 균등 기대치는 브랜치 수에 따라 5/6/7-branch = 40% / 33.3% / 28.6%로 구분해 읽을 것.
>
