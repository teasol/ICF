...
- **같은 실행을 반복해 얻은 동일 점수는 새 증거가 아니다** — 시드 반복을 표본 수로 세지 않는다.
  ...
  *(`D-003`의 "t·p·CI 사용 금지"는 시드 편차 기준의 오적용을 막으려던 것이었고 `D-019`로 폐지됐다.)*
...
> 이 R5·R6은 초안(2026-09-06 오전)의 정정이다. 초안은 *"각 단계가 판별 불가면 합쳐도 판별
> 불가"* 와 *"분해능 하한 아래 후보는 착수 금지"* 를 적었는데, **둘 다 틀렸다.** RU-80 실측에서
> 이 설계의 MDE는 옛 컷오프 `1.0%p`의 **1/14 ~ 1/20**이었다 — v115~v120의 Δ는 판별 불가능한
> 잡음이 아니었다. 경위는 `decisions.md` `D-017`·`D-018`.
