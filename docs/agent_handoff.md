...
3. **활성 Baseline**:
   - **v114 (0-parameter, Deterministic, seed std = 0.00000)**
   - Within-slide PCA (K=256) + CV(off-diagonal 32,640D) + DD(ordered-typicality, κ=1) + CT(1/8 fraction, seeded k-means++ 256 token) + BM(Projected Bag-Mean 32D, w=0.0 default)
...
| 문서 | 역할 및 포함 내용 |
| :--- | :--- |
| [`docs/current_architecture.md`](current_architecture.md) | **현행 아키텍처 완전 명세**: 4대 브랜치(CV, DD, CT, BM)의 수식, 작동 원리, 데이터 흐름, 모듈 패키지 구조 |
