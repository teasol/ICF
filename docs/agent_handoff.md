...
| **§173** HDBSCAN PCA 3D full-cell | 3D로 줄여도 noise 75~97%, SEAL 0.6849/0.6855(v110 대비 −0.0221/−0.0215), 280만 cell 병목. 기각 |
| **§174** random64 + seeded k-means++≤8 | 전체 0.66375. k-means++ 자체는 FPS+30과 동률(+0.00001), 하락은 random sampling에서 발생. working-tree candidate지만 v110 미승격 |
| **§175** PCA64/128 × K256..2048 | 최고 PCA64/K256 0.65744. 동일 K PCA32보다 전부 낮아 PCA32 feature 부족 가설 기각 |
...
4 seed 0.6880과 **절대 수치를 빼면 안 된다**(v98 자신도 A그룹 0.6901 / B그룹 0.6803으로 0.0098
흔들린다). 새 arm은 **v98의 같은 시드와 seed-paired**로 비교한다.
...
> 센터링하면 정확히 사라진다.
>
> **③ K=128은 최적이 아니었다 (§142).** 사영이 학습을 벗어나면서 K가 평가 시점 노브가 됐고,
> 스윕은 역U자였다: 0.6812 / **0.6864**(128) / 0.6909 / **0.6945**(256) / 0.6956 / 0.6948 / 0.6913.
> 전체 17 task +0.0076(t=3.01, 12/17)이고 **선택에 쓰지 않은 홀드아웃 7개에서 재현**된다.
> 런타임은 K와 무관하게 24–25초/task. K=384가 SEAL 10에서는 더 높았으나 홀드아웃에서 뒤집혔다.
>
> ⚠️ **"v98을 이겼다"고 쓰지 말 것.** v107은 v98 8 seed 평균(0.6852) 대비 +0.0093이고 8개 중
> 6개를 이기지만, **상위 두 시드(0.6950, 0.6946)에는 지고** 42–45 부분군으로 좁히면 2/4다.
> 그리고 **v98 4-seed 앙상블 0.6951**(§130)이 아직 근소하게 앞선다(+0.0006, 측정 불가).
> 정확한 요약: 학습 0·결정론 구성이 **49 epoch × 4 시드 학습 후 앙상블한 구성과 같은 자리**에 왔다.
> ⚠️ 위 t들은 seed-paired가 아니다 — v107은 시드 의존이 없어 짝짓기가 분산을 안 줄인다.
...
>
> 세 가지가 연달아 나왔고 서로 맞물린다.
>
> **① 학습된 head 13개는 상수 3개다 (§137).** 최종 logit이 `(−½·margin, +½·margin)`이라 클래스
> 스왑 시 margin이 부호를 뒤집어야 하고, 그 스왑에서 SEP 3개는 **불변**이다 → **라벨 반대칭이
> w(SEP)=0, bias=0을 강제**한다. 차분 feature는 쌍의 선형결합이라 선형 head에 표현력을 안 더한다.
> 남는 것은 분기당 숫자 하나:
> ```
> margin = 1.442·(CV1−CV0) − 0.343·(D1−D0) + 0.286·(q1−q0)     CV:DD:CT = 1 : −0.238 : +0.199
> ```
> 8 seed std가 0.027/0.008/0.012로 소수 둘째 자리까지 같다. head를 이 상수로 갈아끼우면
> **Δ = −0.00014**(최대 편차 0.00060).
> ⚠️ **DD의 음수 부호는 정상이다** — `_dd_distance_features`는 logit이 아니라 프로토타입까지의
> **거리**를 반환한다. 버그로 오해하지 말 것.
>
> **② PCA 사영이 학습된 P와 대등하고 분산은 43배 작다 (§136).** 에피소드 context cell의 상위
> 128 고유벡터를 쓰면 macro 0.6876 vs 학습 P 0.6865(Δ+0.0011, t=0.42), **seed std 0.00725 →
> 0.00017**. §133의 fixed Fourier(−0.0126)와 합치면 **필요한 것은 "학습"이 아니라 "데이터
> 적응성"**이었다 — 학습된 P는 적응성을 비싸게 재발견하며 §134의 random walk 분산을 덤으로 얻는다.
>
> **③ 둘을 합치면 학습 파라미터 0이다 (§137-4).**
> ```
>              P+head   PCA+head   P+fixed   PCA+fixed
> macro        0.6865    0.6876     0.6864    0.6879
> seed std    0.00725   0.00017    0.00721   0.00000
> 학습 param   196,621        13    196,608         0
> ```
> **PCA+fixed는 8 seed가 전부 0.6879로 동일**하다. 그러면 §131-2의 4 seed 검출 한계 0.0121이
> **≈0으로 떨어지고 시드 반복이 불필요해진다** — arm 15개가 미판정으로 끝난 원인이 사라진다.
>
> ⚠️⚠️ **아직 승격 논의를 하지 말 것 (§137-6).** 위 절대값은 `eval_seal_tasks.sh`와 **비교
> 불가**다 — `max_cells` 서브샘플을 로드 시 1회 결정론적으로 적용하고 query마다 재추출하지 않는다.
> **arm 간 비교만 유효**하며, 정식 경로 재측정이 다음 작업이다.
> 재현: `scripts/diagnose_full_basis.py`, `scripts/diagnose_cv_basis.py`.

> [!IMPORTANT]
> **이 문서에서 가장 중요한 3가지 (2026-08-15) — 아래 §115 박스보다 먼저 읽을 것**
>
> **① 데이터 분포 축은 닫혔다 (§129) — 이 축에서 새 arm을 설계하지 말 것.**
> §115(에피소드 **모양**)와 §123(cell **값 분포**) 두 진단이 실제 UNI2와의 격차를 **정확히
> 실측**했고, 그 격차를 닫는 처방은 **전부 실패**했다. 그것도 **닫은 격차가 많을수록 단조로
> 나빠진다**:
> ```
> v83  아무것도 안 닫음         0
> v94  응집도만 닫음          -0.0043   (cos 0.109->0.337, ICC 10.8->34.3%로 실제로 닫았는데도)
> v95  스펙트럼만 닫음        -0.0086   (1 seed)
> v102 둘 다, 단일 기전으로   -0.0130   (t=-2.70, 0/4 -> 게이트 통과 기각)
> ```
> v100(nuisance 최소화)도 t=-3.59, 0/4로 게이트 통과 기각. ⚠️ §125-4에서 **미판정 arm으로 축을
> 닫으려다 한 번 정정**했고, 지금 근거는 **게이트 통과 기각 2건**이다.
> 진단 재현: `scripts/diagnose_synthetic_vs_real.py`(학습 불필요, 수 분).
>
> **② 문제는 편향이 아니라 분산일 수 있다 (§130) — 현재 최전선.**
> macro seed std가 **0.0074**인데 §105 이후 arm 15개가 쫓던 효과는 전부 **±0.005 안쪽**이었다.
> **노이즈보다 작은 신호를 쫓고 있었다.** 반면 **시드 앙상블은 학습 비용 0으로 +0.0058(v83)~
> +0.0071(v98)**, **10/10 task 양수**다. 예측이 전부 디스크에 있어 즉시 계산된다
> (`predictions/pathobench_*_{arm}_seed4{2..5}_ep49_official50_bf16.pt`, fold별 slide 확률 포함).
> ⚠️ model soup(가중치 평균)은 +0.0014로 **실패**했고(P의 시드 간 cos 0.57), arm 다양성도 기여하지
> 않는다(전체 14 arm 0.6913 < v83 시드만 0.6938). 이득은 **시드 분산 감소**에서만 온다.
>
> **③ 판정 절차가 §118에서 바뀌었다.**
> §107-3 게이트(4/4 부호 일치 + `|t|>=2.5`)는 계속 계산·보고하되 **통과 여부가 승격/기각을 자동
> 결정하지 않는다.** 최종 판정은 (a) macro seed-paired Δ+t, (b) **task 10개 전부**를 baseline
> 성능대와 나란히 본 패턴, (c) 다른 arm과의 일관성을 종합한 **사용자 판단**이다.
> **보고 형식**: (i) 이 arm이 뭘 테스트하는지 → (ii) task 10개 전부 표 → (iii) macro Δ+t.
> 요약만 하지 말 것.

⚠️ **1 seed 스크리닝은 "크게 나쁜 것 배제"에만 쓴다 (§125-1).** v94는 seed42에서 **+0.0022**였으나
4 seed로는 **−0.0043**이었다. VHL이 "처음 0.5를 넘었다"(0.5176)던 것도 seed42만이고 나머지 셋은
0.4156~0.4314였다. **양수 부호는 스크리닝으로 주장할 수 없다 — "유망"으로도 쓰지 말 것.**

⚠️ **반증 목적 arm이라도 knob은 한 번에 하나만 (§127-2).** v100이 3개를 묶어 가장 강한 검정을
만들었는데 실패했을 때 **무엇 때문인지 분리되지 않아** v101을 따로 돌려야 했다.

⚠️ **`nvidia-smi`로 GPU 여유를 판단하지 말 것 (§122-5).** caching allocator가 카드를 채우고 캐시를
놓지 않는다 — ×3과 ×2 변형이 **같은** 172.8 GiB에서 평탄화됐고 실제 `max_memory_allocated`는
42.4 GiB였다. 큰 arm은
`PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True,garbage_collection_threshold:0.8`로 띄운다
(실측 peak 138.8 GiB, OOM 0건). **데이터 크기 arm의 비용도 예산 비례로 추정하지 말 것** — cell
3.3배에 step 시간은 +19%였다(78→93 ms).

**남은 실측 단서**: her2/BAP1 **트레이드오프**(저신호 task ↑ / 고신호 task ↓, macro에서 상쇄).
her2는 v101에서 개별 task 게이트를 넘었고(+0.0323, 4/4, **t=+3.09**) 0.6740으로 **ABMIL 0.663을
상회**한다(§128-4). 균등 앙상블로는 양쪽이 1:1로 옮겨와 순효과 0이다(§130-5) — **task별 가중치를
held-out fold로 결정**하는 방법이 미착수로 남아 있다.
...
>
> **⑬ 최종 판정은 통계 게이트가 아니라 사용자의 종합적 판단이다 (§118, 2026-08-14, 사용자 결정)**:
> §107-3의 **4/4 부호 일치 + |t|≥2.5 게이트는 폐지되지 않았다** — 여전히 계산하고 보고한다. 다만
> 이 게이트가 최종 승격/기각을 **자동으로 결정하지 않는다.** v90(class_prior)이 계기다: macro
> Δ−0.0053, t≈−2.23으로 게이트(2.5)에 근소 미달이라 통계적으로는 "미판정"이었지만, **task 10개를
> baseline 성능대별로 다시 보니** 하락한 7개 중 5개(STK11 0.875→0.861, brcaTP53 0.827→0.818,
> er_status 0.728→0.708, EGFR 0.776→0.765, grade 0.726→0.717)가 **천장에 가까워 움직이기 어려운
> 고성능 구간**에 몰려 있었고, 상승한 3개(her2, luadTP53, BAP1)는 전부 **0.5에 가까운 저신호
> 구간**이었다. 사용자가 이 비대칭을 근거로 "게이트 미달이어도 실질적으로 해롭다"고 판단해 v90을
> **기각**했다. **적용 방법**: (a) macro seed-paired Δ+t는 계속 계산·보고한다, (b) **task 10개
> 전부**를 baseline 값과 나란히 보여준다(±방향뿐 아니라 baseline이 0.5 근처인지 1.0 근처인지도
> 함께 — 같은 크기의 Δ라도 천장 근처 변화가 랜덤 근처 변화보다 실질적으로 더 큰 의미를 가질 수
> 있다), (c) **최종 승격·기각·재검증 여부는 사용자가 위 증거를 보고 내린다** — 게이트 통과 여부와
> 사용자의 최종 판단이 다를 수 있다(v90처럼 게이트 미달에도 기각될 수 있고, 반대로 §109처럼
> 게이트 미달에도 baseline으로 승격된 전례도 있다⑨). 이 규칙은 이 리포의 모든 arm 판정에
> 적용되고, 이 리포를 공유하는 모든 노드(NEXGEM 포함)에 동일하게 적용된다.
