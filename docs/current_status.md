...
**Last updated**: `2026-08-04` (**§28 v30 S1 — `poolz` 음성 판정, `poolz_l2` 진행 중 — 최신**)
**Status**: **v24가 확정 baseline (변경 없음)** — v30 B1 플래그는 구현됐으나 **기본 OFF**이며 승격 전입니다. **§28: `poolz`는 게이트 미달로 음성**(합성 0.9506 무회귀지만 Musk **0.776** < 기준선 0.803, n≤4 0.442 무개선, prob-vs-log n **+0.327** 크기 편향). 측정된 원인은 pool 표준화가 1차 적률을 **bag 크기와 교란**시키는 것 → **B2(cardinality-faithful 샘플링)가 B1의 전제조건으로 재확정**. `poolz_l2` 학습 진행 중(기대치 하향). **§26에서 Musk 로드맵을 재설정**했습니다 —
