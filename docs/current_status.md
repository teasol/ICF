...
| **Status** | **D-042 verified** · RU-100 재확증 · RU-101 SJ 무정보 진단 · 오염 검사 L1 재설계 · 자동 잡무 중단(`D-056`) · `SEAL 10` 보류(`D-057`) |
...
- **7-branch `3/7` 악화 기전 부분 확인(RU-101).** SJ 단독 AUROC이 악화 3과제에서 ≈`0.49`(무정보)·나머지 `0.56` → 가중 대상. 인과 분해는 아님(trimmed_mean 상호작용). 기존 후보 `TGW`(P4)에 근거 추가.
