...
**Last updated**: `2026-08-04` (**§28 v30 S1 전체 음성 판정 → S2(B2 cardinality) 착수 — 최신**)
**Status**: **v24가 확정 baseline (변경 없음)** — v30 B1 플래그는 구현됐으나 **기본 OFF**이며 승격 전입니다. **§28: S1(B1 uncentering 단독) 전체 음성** — `poolz` Musk 0.776 / `poolz_l2` 0.762, 둘 다 기준선 0.803 미달(합성은 무회귀). **승격 안 함, `bag_representation` 기본 `legacy` 유지.** 단 실패의 모양이 진단을 지지: `poolz_l2`의 n≤4는 **0.517로 전체 최고**인데 n>34가 0.667→**0.460** 붕괴 → 모델이 **크기 불변성을 배운 적이 없어 두 레지므를 동시에 만족 못 함**. → **S2 = B2(cardinality-faithful log-uniform 샘플링) 착수**. 또한 **선형 ridge 천장은 예측 지표로 폐기**(poolz_l2는 합성 천장 최악인데 실현 최고, Musk 천장 최고인데 실현 최악 — 부호가 양쪽 반대). **§26에서 Musk 로드맵을 재설정**했습니다 —
