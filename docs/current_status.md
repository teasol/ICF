...
> | 순서 | 절 | 왜 읽나 |
> |---|---|---|
> | 0 | **§0** | **판정 프로토콜 종합 SSOT**: 결정론적 판정(t 금지, 부호 일치 수, 독립 재현), 사용자 판단 원칙, 닫힌 축 |
> | 1 | **§190** | **전체 코드베이스 리팩터링 & 테스트 경량화 (335→86 tests, 3.1s)**. 모듈 구조 및 사용 안내 |
> | 2 | **§187** | v114 승격(현 활성). weight 통일, SEAL 0.70509. **홀드아웃·전체 미측정**임을 여기서 확인 |
> | 3 | **§186** | v114 실험 자체 — task 10개 전부와 Δ. 개선분이 저신호 BAP1·VHL에 몰린 근거 |
> | 4 | **§185 (+§184)** | v113 = CT cell 예산 1/8 fraction. **승격 사유가 예측 성능이 아니라 22GB GPU feasibility**다. §185-1이 tokenizer 동시 변경을 정정한다 |
> | 5 | **§183 (+§182)** | v112 = DD ordered × typicality(κ=1). DD weight를 0.343→1.0으로 고친 이유(§182-3) |
> | 6 | **§181** | v111 승격. ⚠️ 그 근거였던 "selection bias·randomness 없음"은 v113부터 무효다(§185-1) |
> | 7 | **§161 → §158 → §152 → §143 → §139** | v110←v109←v108←v107←v106 계보. **§139가 학습을 없앤 지점**이고 §139-4(within-slide 센터링)·§138-4(고정 head)가 그 두 축의 근거다 |
>
> **닫힌 축을 다시 열지 않으려면**: §0-3(요약표) 및 §129(합성 데이터 분포)·§147(DD)·§159/§160(CT cell 수)·§181(CT 탐색)·§188(CT kernel)·§189(CT top-k).
>
