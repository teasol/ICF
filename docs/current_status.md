...
**Last updated**: `2026-08-16` (nhn-NEXGEM-claude 09:36) — §134까지. baseline은 **v98 8 seed 0.6852**(§131). ⚠️ **4 seed로는 0.012 미만을 판별할 수 없다** — 과거 "미판정"은 "측정 불가"다(§131-2). **§133: learnable P는 필요하다**(fixed P −0.0126, t=−3.59 — §105 이후 처음으로 게이트+검출한계 동시 통과) **그리고 seed 분산의 출처가 P다**(seed std 0.00690 vs fixed P 0.00013, 55배). §134: P는 수렴하지 않고 손실이 제약하지 않는 방향으로 random walk 중이다.
...
> [!IMPORTANT]
> **활성 baseline은 v98(§131)이고 공식 SEAL 10-task macro는 1-GPU 8 seed 평균 0.6852다.**
> ```
...
> ```
> ⚠️⚠️ **v98이 v83보다 낫다는 뜻이 아니다.** v98 8 seed 0.6852와 직전 baseline v83 4 seed 0.6880을
> **빼지 말 것** — 서로 다른 시드 집합의 절대 수치다(v98 자신도 A그룹 0.6901 / B그룹 0.6803으로
> 0.0098 흔들린다). v98 vs v83은 4 seed seed-paired로 +0.0021, t=1.73, 4/4 → **여전히 미판정**이다.
> v98을 baseline으로 삼은 근거는 **"8 seed로 측정된 유일한 arm"**이라는 것뿐이다(§131-1).
> **새 arm은 v98의 같은 시드와 seed-paired로 비교한다** — 4 seed면 42–45, 8 seed면 42–49.
>
> 직전 baseline **v83 linear head**(1-GPU 4 seed 0.6880, §109)는 historical이다. ⚠️ v83 승격 자체가
> v82 대비 t=1.15(**p=0.33**)로 **구분된 적이 없다**(§131-3).
>
> 채점은 계속 **epoch 49 고정**(§104-2). 앙상블은 **판정 축에서 분리**하고 최종 산출물에만 얹는다
> (§130·§131-0).
