...
**Last updated**: `2026-08-16` (nhn-NEXGEM-claude 13:15) — §138까지. baseline은 **v98 8 seed 0.6852**(§131). **✅ 확정: 학습된 head 13개는 상수 3개로 대체 가능하다**(정식 경로 Δ−0.0003, §138-4). ⚠️ **PCA 사영은 정식 경로에서 −0.0057로 부호가 뒤집힌다** — §136의 "대등하다"는 8,192 cell 캡 조건 한정이었다(§138-5). ⚠️ **서브샘플링 계약 정리됨(§138-1): relation 계보는 평가 시 tile 캡이 없고(full tile), 학습은 ≤4,096이다.**
