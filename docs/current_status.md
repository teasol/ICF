...
**Last updated**: `2026-08-16` (nhn-NEXGEM-claude 17:40) — §142까지. 활성 구성은 **v107 = within-slide PCA(K=256) + 고정 3상수 head, 학습 파라미터 0**(§142, 사용자 결정, 정식 경로 macro **0.6945**, seed std 0.00000). K는 v106에서 사영이 학습을 벗어나며 **평가 시점 노브**가 됐고, 스윕 결과 128은 최적이 아니었다 — 전체 17 task에서 +0.0076(t=3.01, 12/17), 선택에 쓰지 않은 홀드아웃 7개에서 재현(§142). **독립 최소 구현 `src/models/training_free.py`가 기존 경로와 등가임을 테스트로 고정했다**(§140).
...
>
> ⚠️ v107의 0.6945와 v98 8 seed 0.6852를 **빼지 말 것** — 시드 집합이 다르다(§131-1).
> ⚠️ v106(K=128, 0.6864)은 historical이다. K 비교는 §142 표를 볼 것.
...
`scripts/diagnose_ct_pca_distance.py`. `contrast` = cell 하나의 16개 token 거리에 대한
(max−min)/mean — 집중이 0으로 몰아가는 양. `entropy`는 abundance 행의 엔트로피(ln 16 = 2.773이
완전 균일 = 무용).

| PCA dim | contrast (SEAL / 홀드아웃) | entropy | \|t\| 중앙값 |
|---|---|---|---|
| **1536 (raw)** | **1.370 / 1.378** | 1.228 / 0.870 | 1.31 / 1.23 |
| 4 | 2.287 / **2.919** | 1.768 / 1.651 | 1.17 / 1.39 |
| 8 | 1.946 / 2.521 | 1.371 / 1.142 | 1.35 / 1.39 |
| 16 | 1.956 / 1.902 | 1.118 / 0.868 | 1.36 / **1.58** |
| 32 | 1.754 / 1.700 | 0.984 / 0.693 | 1.31 / **1.60** |
| 256 | 1.542 / 1.422 | 0.902 / 0.851 | 1.23 / 1.21 |

**두 집단에서 일관되게, raw 1536의 contrast가 가장 낮다.** 차원을 줄이면 1.37 → 최대 2.92로
2배 이상 벌어지고, soft assignment가 날카로워지며(entropy 감소), §148-4가 증상으로 잡았던
token별 판별 통계도 오른다(홀드아웃 1.23 → 1.60). **지적한 기전이 그대로 측정된다.**
...
- **지적은 옳았다**: 거리 집중은 실재하고(contrast 1.37 → 2.9), CT의 표현을 실제로 망치고 있었다.
...
- §148-5의 남은 두 후보(**farthest-point token 생성**, **64-cell 샘플링**)는 여전히 미검증이다.
  contrast가 개선돼도 |t| 중앙값이 1.6을 못 넘는다는 것은 **거리만이 문제가 아니라는** 신호다.
