...
>
> ⚠️ **"v98보다 낫다"가 아니다.** 같은 시드 seed-paired Δ는 **−0.0037**(t=−1.08, 1/4 → 측정 불가,
> 부호는 음수)이다. macro를 조금 내주고 **학습 제거·시드 반복 제거·§131-2 검출 한계 제거**를
> 얻는 트레이드이며, 사용자가 그 트레이드를 선택했다(§139-2).
>
> ⚠️ v106의 0.6864와 v98 8 seed 0.6852를 **빼지 말 것** — 시드 집합이 다르다(§131-1).
