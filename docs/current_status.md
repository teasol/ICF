...
```bash
bash scripts/ops/lit_fetch.sh <url> <slug>    # curl로 디스크 직행 (읽지 않는다)
# 요약은 잡무 큐에 넘긴다: lit_digest.py --slug <slug>
cat talks/lit/INDEX.md                        # 무엇을 읽었는지 찾는 곳
```

- **수집은 외부 접근이 가능한 세션(소집자)만** 할 수 있고, 요약·판단은 로컬 모델이 한다.
  `curl`로 받으므로 원문이 소집자 컨텍스트에 들어가지 않는다.
- PDF는 `pdftotext -layout`으로 변환한다. 표의 열 정렬을 유지해야 결과 수치를 읽을 수 있다.
- **브리프는 저장소에 넣고 원문(`talks/lit/raw/`)은 넣지 않는다.** 원문은 수백 KB이고
  기록된 URL로 다시 받을 수 있다. 브리프는 작고 GPU를 써서 만든 것이다.
- `INDEX.md`는 코드가 생성한다. 모델이 브리프를 쓰고 색인까지 쓰면, 확신에 찬 오요약을
  가장 알아차리기 어려운 곳에 두는 셈이 된다.
- **모든 브리프는 외부 주장이며 우리의 관측이 아니다**(`D-047`).
...
**잡무와 실험을 구분한다.** 판단 기준은 **"아무도 읽지 않아도 되는가"** 다.

| | 잡무 (큐에 넣는다) | 실험 (Claude가 직접 띄운다) |
|---|---|---|
| 산출 | 점검 결과·목록. 연구 주장 없음 | **연구 증거** |
| 실행 | 감독자가 유휴 시 자동 | 의도적으로, 사전 설계와 함께 |
| 결과 | 파일에 누적, 필요할 때 봄 | **반드시 읽고 기록·해석** |
| 자원 | vLLM(좌석 서버) 또는 CPU, 짧게 | GPU 4, 길게 |

**잡무는 `talks/ops/recurring/`에 주기와 함께 등록한다.** 한 번 쓰고 `done/`으로 가는
단발 항목은 잡무가 아니다 — 2026-09-18에 큐가 1분 만에 비고 30분간 아무것도 돌지 않았다.
감독자가 매 틱 주기를 확인해 큐로 되돌린다(`every_minutes`). 병렬 가능한 항목은
`"parallel": true`로 표시하며, 동시 실행 수에 상한을 두지 않는다 — vLLM은 요청을 거부하지
않고 큐잉하므로 실제 한계는 각 항목의 타임아웃이다.

실험을 큐에 넣으면 **아무도 읽지 않은 실측**이 쌓인다. 이 프로젝트는 이미 그 병을 앓았다 —
2026-09-16 GF 실행 7건이 이틀 동안 어느 문서에도 반영되지 않은 채 남아 있었다.
