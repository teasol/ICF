...
후속 설정은 `train_learnability_c4.yaml`(composition+nuisance), `train_learnability_c5.yaml`(C4+rare-effect)이다. C4 이후에도 C와 동일하게 warm-up 이후 full-LR 3 epoch를 판정 구간으로 사용한다.
...
`C4-N`만 실패하면 nuisance group을 global shift, component shift, mixture-logit variability 순으로 분해한다. `C4-D`만 실패하면 response margin/effect scale을 먼저 sweep하고 component heterogeneity와 크기 범위를 나중에 분리한다. 둘 다 통과할 때만 결합 interaction을 검사한다. C5 rare-effect와 이후 전체 task 확장은 이 경계를 통과한 뒤 실행한다.
...
두 축은 각각 학습 가능하다. 다음 단계는 medium difficulty와 nuisance를 함께 결합하되 composition-only와 rare off를 유지하여 interaction을 검사하는 것이다. 결합 조건을 통과한 뒤 C5에서 rare effect를 추가한다.

## D0–D4 nuisance ablation

C4-D는 nuisance-off 기준 실험으로 독립 유지하며 D0–D4는 다섯 nuisance를 각각 단독으로 활성화한다. 모든 stage는 online episode, composition-only, medium difficulty, episode별 nonlinear manifold, random query, rare-effect off, CE-only, AdamW LR `5e-4`를 공유한다. `return_oracle_diagnostics: true`이며 oracle scalar는 모델 입력이나 loss에 사용하지 않는다.
...
D0–D4는 `configs/train_learnability_c4_d.yaml`을 상속하고 nuisance key 하나만 override한다. `configs/train_learnability_c4.yaml`은 다섯 nuisance를 모두 켠 C4 실험이다.
...
model과 oracle AUROC가 같이 내려가면 generator separability 감소로 해석한다. oracle은 유지되고 model만 내려가면 architecture nuisance 처리 문제다. D2 하락은 abundance signal과 fraction noise의 충돌, D0/D1 gap 증가는 embedding shift invariance, D3/D4 gap 증가는 mixture 변화에 따른 slot alignment 문제를 우선 조사한다.
...
실행 결과와 비교 표는 [C4-D 및 D0–D4 nuisance ablation 결과](nuisance_ablation_c4_d_d0_d4.md)에 정리했다.
