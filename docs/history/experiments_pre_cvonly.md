...
**아카이브됨 (2026-08-09).** CV-only 이전의 실험 프로토콜(v22~v39, 합성 중심 판정).
현행은 [`../current_experiments.md`](../current_experiments.md).

> [!WARNING]
> 이 문서의 판정 방식은 **폐기됐다**. 합성 val_ce·val_AUROC로 arm을 고르는 절차가
> 전반에 깔려 있는데, SS65·SS69-6에서 그 지표가 실제 성능과 어긋남이 반복 확인됐다.
> 현재 판정 기준은 **SEAL 10개 task macro 평균**이다(SS71-4).
