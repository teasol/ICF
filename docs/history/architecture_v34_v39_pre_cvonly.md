...
**아카이브됨 (2026-08-09).** 이 문서는 **CV-only 이전**(v34~v39)의 6-분기 아키텍처를 기술한다.
§68에서 6개 evidence 분기 중 4개(global_shape, population, rare, fusion interaction)가
제거해도 동률임이 확인되어 폐기됐다. 현행 명세는 [`../current_architecture.md`](../current_architecture.md).

> [!WARNING]
> 알려진 오류: §5의 `covariance_matrix는 (C, 64, 64)`는 **틀렸다**. 당시
> `_projected_covariance_matrix`의 기본 인자가 32라 실제로는 32×32였다(SS69-7).
