...
### RU-98. 결론 수준 교차 기계 재현성 계측

- **일자 (Date)**: `2026-09-19` ~ `2026-09-19`
- **커밋 범위**: - (`20a22364` ... `6f6d5ede`)
- **작업 유형**: `reproduction`
- **질문 (Question)**: 같은 코드·같은 config·같은 데이터로 5-branch Primary 7을 다른 장비(Slurm 클러스터 GPU 노드)에서 돌렸을 때, 2026-09-18 NHN NEXGEM(GPU 4)에서 얻은 과제별 fold-mean AUROC이 재현되는가. 표류 크기는 PROJECT.md 3.4의 4자리 요구 및 승격 임계 +0.0030과 비교해 어디에 놓이는가.
- **가설 (Hypothesis)**: 부동소수 표류는 float32 반올림 규모(집계 margin 기준 절대 1e-6 수준, 합성 입력 실측)이므로 AUROC 4자리에는 거의 영향을 주지 않을 것이다. 기전은 BLAS 커널·벡터화 폭·리덕션 순서 차이로 추정하며 미확인이다. 앵커 어긋남 5e-4는 이보다 두 자릿수 크므로 장비가 원인이 아닐 것으로 본다 — 이것도 이 RU가 판별 대상으로 삼는다.
- **판정 기준 (Criteria, 사전 고정)**: 결과를 보기 전에 고정한다. 과제별 |ΔAUROC|의 최댓값 기준: (1) < 0.0005 이면 4자리 정밀도가 장비를 건너 유지된다 — 앵커 5e-4 어긋남은 장비 탓이 아니다. (2) 0.0005 이상 0.0030 미만이면 4자리 값 앵커는 장비 간 성립 불가이나 승격 판정은 안전하다. (3) 0.0030 이상이면 장비가 승격 판정을 뒤집을 수 있어 허용 오차를 판정 규칙에 넣는 별도 결정이 필요하다. 부호 분포도 함께 본다 — 한쪽으로 쏠리면 계통 차이, 흩어지면 반올림.
- **예산 / 중단 조건**: 약 1.94 GPU-hour (350 fold). array 원소당 --time=04:00:00. 초과 시 중단하고 보고한다. / 선행 조건인 데이터 동일성이 깨지면 착수하지 않는다(2026-09-19 확인 완료: feature 파일 수·표본 sha256·fold manifest 내용 해시 전부 일치). 드라이버가 cu130에 못 미치는 노드에 배치되면 그 원소는 즉시 실패하고 해당 노드를 --exclude 하여 재제출한다. 예산 초과 시 중단.
- **실험 및 변경 (Experiment)**: scripts/slurm/xmachine_repro.sbatch · config configs/baseline/v121_active.yaml (sha256 2a2ec33a97ab3e45) · 과제 7개 x fold 50 · 과제 하나가 array 원소 하나 · GPU 1장씩 병렬 · 배치는 Slurm에 맡기고 노드·GPU 모델·드라이버를 job 로그에 기록.
- **관측 결과 (Observations)**: Slurm array 156357의 7개 원소가 모두 COMPLETED, ExitCode 0:0으로 종료됐다. 과제별 fold-mean AUROC은 B200 참조와 표시 정밀도 4자리에서 7/7 동일했다: ARID1A 0.5507, Histologic_Grade 0.6772, KEAP1 0.6038, KRAS 0.7004, SMAD4 0.4426, progression_regression 0.7891, PBRM1 0.5546; macro 0.6169. 양쪽 값이 같은 4자리 반올림 구간에 있으므로 과제별 실제 |Δ|는 0.0001 미만이며 사전 기준 0.0005 미만에 들어간다. 실행 보고: talks/reports/2026-09-19_cross_machine_repro.md.
- **결정 (Decision)**: 판정 — 지지(supported) · 운영 결정: 종료. 사전 등록 기준 (1)을 충족한다: 5-branch Primary 7 과제별 fold-mean AUROC이 B200 참조와 표시 정밀도 4자리에서 7/7 동일하므로 각 과제의 실제 |ΔAUROC| < 0.0001이며 첫 기준 |Δ| < 0.0005에 들어간다. 부호 분포는 4자리에서 7개 모두 0이라 계통 편향을 볼 수 없다. 따라서 '4자리 정밀도가 장비를 건너 유지된다'와 '앵커 5e-4 어긋남은 장비 탓이 아니다'가 이 실행 범위에서 지지된다. 판정 주체: 사용자 위임에 따라 오케스트레이터/종결권자 자격으로 opencode가 판정했다. RU를 설계한 주체와 다른 세션이며, 판정의 독립성은 모델·문맥 분리로 부분적으로만 확보된다. 기준 (2)·(3)은 발동하지 않았고, 승격 판정 허용 오차를 판정 규칙에 넣을지는 별도 사용자 판단 사안으로 남긴다.
- **결과별 후속 행동**: sbatch --array=0-6 --wait scripts/slurm/xmachine_repro.sbatch 실행 후 과제별 AUROC을 talks/reports/2026-09-18_arm_parity.md 의 5-branch 참조값과 대조하고 talks/reports/2026-09-19_cross_machine_repro.md 에 provenance와 함께 기록한다.
- **원문 근거 (Evidence)**:
  - 기계 A 참조값: talks/reports/2026-09-18_arm_parity.md (5-branch, macro 0.6169). 코드 동등성: 패리티 실행 시점 830c973과 HEAD의 집계 margin이 이 기계에서 비트 동일(5branch 454a2effff16b2db, 7branch bf8f25ee1ff0ab9d) — 그 사이 src/ 변경 e2b0d79·bda3503은 집계 경로에 영향 없음. 데이터 동일성 대조 결과는 kill 항목 참조.
- **선행·후속 관계 (Relations)**: PROJECT.md 3.4(오염 검사 앵커) · D-050(provenance 기반 재설계) · D-053 · talks/reports/2026-09-18_arm_parity.md · 승격 임계는 PROJECT.md 4.
...
- **일자 (Date)**: `2026-09-19` ~ `2026-09-19`
- **커밋 범위**: - (`b3b7fb7f` ... `b3b7fb7f`)
- **작업 유형**: `confirmatory`
- **질문 (Question)**: 같은 잡·같은 세션에서 5-branch(v121_active)와 7-branch(v121_7branch_active)를 Primary 7 × 50 fold로 각각 실행하고, provenance를 기록한 뒤 과제별 대응 per-fold Δ의 macro 평균이 PROJECT.md §4 점추정 규칙(+0.003)을 충족하는가? 과제군집 95% t 구간(df=6)·SE·sign agreement·악화 과제 전량은 무엇이며, 7-branch 실측 초/fold는 얼마인가? D-042의 7-branch 승격 근거를 unverified-hold에서 verified로 올릴 수 있는가?
- **가설 (Hypothesis)**: arm_parity(2026-09-18)의 점추정 Δ_macro = +0.0057이 이 재실행에서도 재현되어 §4 규칙을 충족할 것이다. 과제군집 95% 구간은 0을 포함하고 3/7 과제가 악화될 것으로 본다 — 즉 판정은 규칙상 '충족'이고 통계적 구분은 '판별 불가'로 남을 것이다. 기전은 미확인.
- **판정 기준 (Criteria, 사전 고정)**: 결과를 보기 전에 고정한다. 주 판정은 PROJECT.md §4 점추정 규칙만 쓴다: Δ_macro ≥ +0.003이면 기준 충족, 미만이면 미충족. CI 하한을 합격 조건으로 넣지 않고 sign agreement에 임계를 걸지 않는다. 함께 보고: 과제군집 95% t 구간(df=6), SE, sign agreement, 악화 과제 전량과 폭, fold 단위 짝지은 Δ의 sd, 7-branch 실측 초/fold. 집계 단위는 과제 군집으로 고정하고 fold 독립 가정 구간은 산출만 하고 판정에 쓰지 않는다. 실행 무효 조건: NaN/Inf, fold 누락, provenance 필드 누락 또는 두 arm의 manifest·config·code 해시 불일치, 4 GPU-h 초과, 같은 잡 내 상주화 ON/OFF fold AUROC 비트 불일치.
- **예산 / 중단 조건**: 최대 4 GPU-hour (7 과제 × 2 arm × 50 fold = 700 fold 적합). 초과 시 중단하고 보고한다. / 다음 중 하나면 즉시 중단하고 실행 무효로 기록한다: Primary 7 중 한 과제라도 fold 누락 또는 NaN/Inf, 두 arm의 manifest_hash·config_sha256·code_hash 불일치, provenance 미기록, 4 GPU-h 초과. 기준선 arm을 재사용하지 않고 같은 잡에서 두 arm을 모두 돌린다.
- **실험 및 변경 (Experiment)**: scripts/slurm/primary7_arm_confirmation.sbatch — array 0-6(과제 하나당 원소), 각 원소가 5-branch(v121_active, sha256 2a2ec33a97ab3e45)와 7-branch(v121_7branch_active, sha256 d8c8ae4dca9b8ecf)를 같은 노드·GPU·세션에서 순차 실행하고 결과를 predictions/parity_* 이름으로 저장. gnode6에서 2회 실행(job 156899, provenance 정정 후 156906). evaluate_pure.py가 provenance를 저장(양 arm 동일 manifest·config·code). 7-branch 단가는 전용 잡 156914(scripts/slurm/ru100_7branch_cost.sbatch)로 따로 측정했다.
- **관측 결과 (Observations)**: 과제별 fold-mean(5br/7br): ARID1A 0.5507/0.5795, Grade 0.6772/0.6665, KEAP1 0.6038/0.6248, KRAS 0.7004/0.7031, SMAD4 0.4426/0.4661, progression 0.7891/0.7772, PBRM1 0.5546/0.5408. macro 5br 0.6169, 7br 0.6226. paired Δ_macro = +0.0057 ≥ +0.003 → PROJECT.md §4 점추정 규칙 충족. 과제군집 95% t 구간(df=6) [-0.0115, +0.0228] 0 포함, sd 0.0185, SE 0.0070, 악화 3/7(Grade -0.0107, progression -0.0120, PBRM1 -0.0139), 임계 대비 하한 -3.8배. fold 독립 가정 구간 [+0.0016,+0.0097]은 판정에 쓰지 않는다. provenance: 14개 파일 전부 10키 실재, branch_list 5/7(shj 별칭 미포함), 두 arm 해시가 manifest·config·code에서 일치. 7-branch 단가 2.0 초/fold(전용 H100, SMAD4 fold 10). array 시각은 7원소 동시 실행의 경합으로 오염되어 단가로 쓰지 않는다. arm_parity의 삭제됐던 parity_* 원시 예측이 이 실행으로 복원됐다.
- **결정 (Decision)**: 지지 · 채택 · 종료. §4 점추정 규칙상 Δ_macro +0.0057 ≥ +0.003으로 기준 충족이 재현됐고, provenance가 기록되어 D-042 7-branch 승격 근거를 unverified-hold에서 verified로 올린다. 통계적 구분(과제군집 CI가 0 포함, 3/7 악화)은 판별 불가로 남으며 판정을 막지 않되 함께 보고한다. 기전 미확인. 이 RU로 확증 절차는 '적합 분산'이 아니라 provenance 고정 재실행임이 실행으로 확정됐다.
- **결과별 후속 행동**: 충족 시: D-042 근거를 verified로 올리고 current_status·PROJECT.md에 반영하며, 실측 7-branch 초/fold로 남은 예산을 재계산한다. 미충족 시: D-042 재검토를 Reasoning/사용자에게 올린다(강등 규칙 부재, T2). 어느 경우든 판정은 §4 점추정 규칙만으로 하고 CI·sign은 보고만 한다.
- **원문 근거 (Evidence)**:
  - scripts/slurm/primary7_arm_confirmation.sbatch
  - scripts/slurm/ru100_7branch_cost.sbatch
  - predictions/parity_*_v121_{active,7branch_active}.pt (14개, provenance 포함)
  - scripts/analysis/parity_paired_delta.py
  - slurm_outputs/2026-09-19/1736,1753/armconf-*.out
  - slurm_outputs/2026-09-19/1757/ru100cost.out
  - talks/reports/2026-09-18_arm_parity.md
  - docs/history/archive.md D-042·D-054
- **선행·후속 관계 (Relations)**: 선행 협의체 C-20260919-1(승격 확증 절차=provenance 고정 재실행) · RU-99(상주화, 700 fold≈1.38 GPU-h) · RU-98(교차 기계 4자리) · talks/reports/2026-09-18_arm_parity.md · D-042(7-branch 승격) · D-050(provenance) · D-054(R=1, 점추정 규칙 유지). 승격 기준은 PROJECT.md §4.
...
### RU-102. TGW 착수 가치 판정 — oracle per-task 상한 대비

- **일자 (Date)**: `2026-09-19` ~ `2026-09-19`
- **커밋 범위**: - (`eae04a33` ... `eae04a33`)
- **작업 유형**: `diagnostic`
- **질문 (Question)**: TGW(TGW=P4 #16, Context 신호 기하로 형상 브랜치 가중)에 착수할 가치가 있는가? 저장된 per-branch margin(RU-101)으로 과제별로 브랜치 부분집합을 **oracle로** 고를 때의 macro 상한이, 승격 기준 `+0.003`과 과제군집 SE `0.0070`(RU-100) 대비 얼마인가? 상한이 승격 기준 미만이면 어떤 현실적 context-only 신호로도 승격에 못 미친다는 뜻이다.
- **가설 (Hypothesis)**: RU-101에서 SJ가 악화 3과제에서만 무정보였다. 따라서 7-branch 대비 {SJ 제거, SH 제거, 둘 다 제거} 중 과제별 최선을 고르는 oracle 상한은 `+0.003`~`+0.006` 정도로 작고, 잡음(SE 0.0070)과 구분되지 않을 것이다. 그러면 TGW는 착수 가치가 낮다.
- **판정 기준 (Criteria, 사전 고정)**: 결과를 보기 전에 고정한다. 후보 family: F7(7-branch), F6-SJ, F6-SH, F5(둘 다 제거), 그리고 단일 브랜치 중 과제별 최선. oracle = 각 과제에서 family를 **사후 선택**(query AUROC 최대화)한 macro. **판정 규칙**: (상한_Δ = oracle_macro − 0.6226) < `+0.003`(승격 임계)이면 **TGW 착수 보류 권고**, ≥ `+0.003`이면 설계 진행. 상한이 임계를 넘더라도 과제군집 SE 0.0070과 비교해 '잡음과 구분 불가'를 함께 보고한다. oracle은 query 라벨을 쓰므로 **유효한 방법이 아니라 상한**이다 — 확증·승격에 쓰지 않는다. 실행 무효: margin 누락, 재집계 macro가 0.6226과 불일치.
- **예산 / 중단 조건**: 0 GPU-hour (저장 margin 오프라인 재집계, RU-101 산출물). / 저장 margin에서 family 재집계가 불가하거나 재구성 macro가 공식 0.6226과 다르면 실행 무효로 종료한다. 상한이 임계 미만이면 그 자리에서 보류 권고로 종료하고 추가 GPU를 쓰지 않는다.
- **실험 및 변경 (Experiment)**: 저장된 RU-101 per-branch margin(predictions/margins_7b, 7-branch, provenance)을 오프라인 재집계. family {F7, F6-SJ, F6-SH, F5}와 단일 브랜치를 과제별로 구성하고, 스크립트 scripts/analysis/tgw_oracle_ceiling.py로 조회했다. GPU 0.
- **관측 결과 (Observations)**: F7 macro 재구성 0.6226(공식 일치). 과제별 family AUROC: PBRM1 F5 0.5546 · ARID1A F7 0.5795 · Grade F5 0.6772 · KEAP1 F7 0.6248 · KRAS F6-SJ 0.7039 · SMAD4 F7 0.4661 · progression F5 0.7891. **oracle family macro 0.6279, Δ = +0.0053** (임계 +0.003 초과). 악화 3과제(PBRM1·Grade·progression)가 모두 F5(SH·SJ 둘 다 제거)를 고른다 — RU-101과 일관. oracle best-single macro 0.6596, Δ=+0.0371(단일 브랜치는 앙상블을 떼어낸 선택이라 과적합·비현실).
- **결정 (Decision)**: 사전 규칙 적용: Δ_fam `+0.0053 ≥ +0.003` → **TGW 설계 진행**. 단 두 한계를 함께 기록한다 — (1) oracle은 query 라벨로 family를 고른 **낙관 상한**이지 방법이 아니다. (2) Δ가 과제군집 SE `0.0070`보다 작아 **잡음과 구분 불가**이며, context-only 신호가 회수할 몫은 상한보다 작다. 따라서 다음 RU는 'context-only 신호가 상한의 일부라도 회수하는가'를 전이 검증으로 판정한다. 종료.
- **결과별 후속 행동**: 상한 < 임계: TGW를 큐에서 '보류(상한 미달)'로 표시하고 착수하지 않는다. 상한 ≥ 임계: context-only 신호 설계(전이 검증 포함)를 다음 RU로 열고, 사전 등록 후 소액 GPU로 확인한다.
- **원문 근거 (Evidence)**:
  - scripts/analysis/tgw_oracle_ceiling.py
  - predictions/margins_7b/*.pt
  - scripts/analysis/task_branch_diagnosis.py
  - docs/research_directions.md P4 #16
- **선행·후속 관계 (Relations)**: 선행 RU-101(SJ 무정보 관측) · RU-100(7-branch macro 0.6226, 과제군집 SE 0.0070) · research_directions P4 #16(TGW) · closed_axes CA-08(집계 변형, 탐색 예외).
- **확인 필요 사항 및 한계 (Uncertainties)**: oracle은 query 라벨로 family를 고르므로 하한이 아니라 낙관 상한이다. family를 {F7,F6-SJ,F6-SH,F5} 4개로 제한하므로 그보다 넓은 부분집합 탐색의 상한은 다를 수 있다(넓히면 상한이 커지나 과적합도 커진다). '단일 브랜치 최선' family는 브랜치를 앙상블에서 떼어내는 것이라 실현 가능성이 낮다. margin은 7-branch 실행에서 나온 것이라 5·6-branch를 margin 재집계로 흉내낸다.

---

### RU-103. TGW context-only 신호의 Context→Query 전이 검증

- **일자 (Date)**: `2026-09-19` ~ `2026-09-19`
- **커밋 범위**: - (`e04eaf25` ... `e04eaf25`)
- **작업 유형**: `confirmatory`
- **질문 (Question)**: Context 라벨만으로 얻는 신호 — 각 형상 브랜치(SH·SJ)의 **context 집합 자기 AUROC**(context가 곧 질의) — 가 query 성능을 예측하는가? 사전 고정 규칙 R1: fold마다 형상 브랜치의 context AUROC < 0.5이면 그 브랜치 가중을 0으로, 아니면 1로 두고 trimmed_mean을 재집계한다. R1의 paired fold Δ_macro가 승격 임계 `+0.003` 이상인가?
- **가설 (Hypothesis)**: RU-102 oracle 상한(+0.0053)은 query 라벨을 쓴 것이고, context 자기 AUROC는 악화 3과제에서 낮게 나올 것이다. 그러나 context→query 전이는 약해 R1의 실현 Δ는 `+0.003`에 못 미칠 것으로 본다(전이 실패 가능성). 기전 미확인.
- **판정 기준 (Criteria, 사전 고정)**: 결과를 보기 전에 고정한다. 신호: fold마다 `clf.branch_margins(context_bags, context_labels, context_bags)`로 context 슬라이드 자신의 브랜치 margin을 얻어 context 라벨과의 AUROC을 계산한다(쿼리 라벨 미사용 — 누출 없음). 규칙 R1(사전 고정): SH·SJ 각각 context AUROC < 0.5 → 가중 0, ≥ 0.5 → 1. 그 가중으로 trimmed_mean 재집계. 주 판정: 같은 fold에서 R1과 F7을 짝지어 Δ_macro를 구하고, `Δ ≥ +0.003`이고 과제군집 SE `0.0070`과 비교해 방향을 보고한다. 성공/실패 외에 '신호는 있으나 전이 없음'을 구분해 기록한다. 실행 무효: fold 누락, NaN, provenance 누락, 재구성 F7 macro가 0.6226과 불일치. R1 외의 규칙을 사후에 추가하지 않는다(추가하면 탐색으로 표시).
- **예산 / 중단 조건**: 최대 0.5 GPU-hour (Primary 7 × 50 fold context+query margin 덤프, H100 실측 2.0초/fold 기준 약 0.2 GPU-h + 여유). / context/query margin이 저장되지 않거나 provenance가 누락되면 실행 무효. F7 재구성이 0.6226과 다르면 실행 무효. R1이 임계 미달이면 그 자리에서 TGW를 기각하고 추가 탐색 규칙을 만들지 않는다(추가하면 별도 RU).
- **실험 및 변경 (Experiment)**: scripts/slurm/ru103_ctxq_margins.sbatch (job 157078, gnode6 H100, 635초=0.176 GPU-h): dump_context_query_margins.py로 Primary 7 × 50 fold의 context 자기 margin과 query margin을 branch별로 덤프. 이어 scripts/analysis/tgw_transfer_test.py로 사전 고정 규칙 R1을 오프라인 적용.
- **관측 결과 (Observations)**: F7 macro 재구성 0.6226(공식 일치). context 자기 AUROC: SH 0.824~0.993(평균 0.929), SJ 0.519~0.748(평균 0.627), 350 fold 전부 ≥0.5. 따라서 규칙 R1은 SH·SJ를 **한 번도 제거하지 않았고**(drop 0.00/0.00), R1 ≡ F7, Δ = +0.0000. F7 재구성 0.6226.
- **결정 (Decision)**: 판별 불가 · 종료. R1이 발동하지 않아 context→query 전이를 **시험하지 못했다.** Δ=0은 '전이 실패'가 아니라 규칙 no-op의 결과다. 사전 kill('R1 미달 → TGW 기각')을 기계적으로 적용하지 않았다 — 근거 없이 기각하면 '효과 없음'과 '판별 불가'를 혼동한다(AGENTS §7). 원인은 context 자기 AUROC이 **in-sample**(자기 자신을 예측)이라 훈련 정확도처럼 부풀려져 0.5를 넘지 못하는 구성 결함이다. TGW는 큐에 남기되 'context-only 신호 구성 실패'로 표시하고, 유효한 신호(context 내 held-out/LOO AUROC, 데이터 기반 임계) 재설계는 별도 RU로 분리한다. 사후 규칙 추가는 하지 않았다.
- **결과별 후속 행동**: R1 ≥ +0.003: context-only 신호 전이 확인 — TGW를 설계 후보로 승격 검토에 올린다(승격은 T2). R1 < +0.003: TGW를 'context→query 전이 실패'로 기각하고 큐에서 내리며, 그 관측을 기록한다. 신호는 있으나 전이 없음이면 그 구분을 남긴다.
- **원문 근거 (Evidence)**:
  - scripts/analysis/dump_context_query_margins.py
  - scripts/analysis/tgw_transfer_test.py
  - scripts/slurm/ru103_ctxq_margins.sbatch
  - predictions/ctxq_7b/*.pt
  - slurm_outputs/2026-09-19/1932/ru103-157078.out
  - docs/research_directions.md P4 #16
- **선행·후속 관계 (Relations)**: 선행 RU-101(SJ 무정보) · RU-102(oracle 상한 +0.0053) · research_directions P4 #16(TGW) · closed_axes CA-08(제한된 결합 연구 예외·선택 이력 요건).
- **확인 필요 사항 및 한계 (Uncertainties)**: context 자기 AUROC은 라벨 사용이 아니라 in-context 자기평가이며 쿼리 라벨을 쓰지 않는다. R1은 임계 0.5의 경성 규칙 하나로 사전 고정한다 — 연속 가중·다른 임계는 사후 탐색이므로 하지 않는다. 전이 실패는 신호 자체의 부재인지 규칙의 거칠음인지 구분되지 않을 수 있으며, 그 경우 '판별 불가'로 적는다.

---

### RU-104. label-free context 합의 신호의 전이 검증 (SJ/SH 하향)

- **일자 (Date)**: `2026-09-20` ~ `2026-09-20`
- **커밋 범위**: - (`9ca549a0` ... `9ca549a0`)
- **작업 유형**: `confirmatory`
- **질문 (Question)**: RU-103이 건진 context-side margin만으로(라벨 미사용) 형상 브랜치 하향 신호를 만들 수 있는가? 사전 고정 규칙 R2: fold마다 context 슬라이드에서 기저 5브랜치(cv·bm·bd·qa·ds)의 trimmed-mean 확률(합의)과 형상 브랜치(SH·SJ) 확률의 **상관**을 계산해, 상관 < 0이면 그 형상 브랜치 가중을 0으로, 아니면 1로 두고 query를 재집계한다. R2의 paired fold Δ_macro가 `+0.003` 이상인가?
- **가설 (Hypothesis)**: RU-103의 in-sample 자기 AUROC은 퇴화했으나, 기저 합의와의 **상관**은 다른 신호다. 악화 3과제에서 형상 브랜치가 기저와 덜 일치(낮은/음의 상관)할 것으로 본다. 다만 상관의 크기가 과제 간에 겹치면 전이는 약해 Δ가 임계 미달일 것이다. 기전 미확인.
- **판정 기준 (Criteria, 사전 고정)**: 결과를 보기 전에 고정한다. 신호: context_margins(RU-103 덤프)만 사용, 라벨 미사용. 합의 = 기저 5브랜치 sigmoid 확률의 trimmed_mean(최저·최고 1 절사). 형상 브랜치 신호 = context 슬라이드에 대한 corr(sigmoid(형상 margin), 합의). 규칙 R2(사전 고정): corr < 0 → 가중 0, ≥ 0 → 1. SH·SJ 각각 독립 적용. 주 판정: 같은 fold에서 R2와 F7을 짝지어 Δ_macro를 구하고 `Δ ≥ +0.003`이면 전이 확인. 함께 보고: 과제별 Δ, 형상 브랜치 drop율, corr 분포(악화 vs 나머지). 실행 무효: fold 누락, NaN, F7 재구성이 0.6226과 불일치. R2 외 규칙·임계는 사후 추가하지 않는다(추가하면 탐색으로 표시).
- **예산 / 중단 조건**: 0 GPU-hour (RU-103의 predictions/ctxq_7b 오프라인 재집계). / context/query margin 누락, F7 재구성 불일치면 실행 무효. R2가 임계 미달이면 TGW 계보를 '전이 실패'로 종료하고 추가 규칙을 만들지 않는다. R2가 한 번도 발동하지 않으면(퇴화) 판별 불가로 적고 기각하지 않는다.
- **실험 및 변경 (Experiment)**: RU-103의 predictions/ctxq_7b(context 자기 margin + query margin)를 오프라인 재집계. scripts/analysis/consensus_transfer_test.py로 사전 고정 규칙 R2(corr(형상, 기저 5브랜치 합의) < 0이면 그 형상 브랜치 가중 0)를 적용. GPU 0.
- **관측 결과 (Observations)**: F7 macro 재구성 0.6226(공식 일치). context 상관(형상 브랜치 확률 vs 기저 5브랜치 trimmed-mean 확률): SH +0.55~+0.80, SJ +0.12~+0.28 — **전 과제에서 양수**이며 악화 3과제(PBRM1 sj +0.13, Grade +0.18, progression +0.25)가 나머지와 구분되지 않는다. R2는 KRAS에서만 SJ를 6% fold 하향했고, R2 macro 0.6225, **Δ = −0.0001**.
- **결정 (Decision)**: 반박 · 종료(전이 실패). label-free context 합의 신호는 형상 브랜치를 하향할 지점을 가르지 못한다 — 형상 브랜치는 in-context에서 기저와 일관(양의 상관)인데도 query에서 3과제를 악화시킨다. 즉 **context→query 브랜치 유용성 전이가 성립하지 않는다.** RU-103(자기 AUROC 퇴화)·RU-102(oracle 상한이 SE 미만)·RU-104(합의 신호 실패)를 합쳐 TGW 계보를 종료한다. 기전 미확인이며, P5 #22(과제 판별 선택 신호 부재) 교착과 일치한다.
- **결과별 후속 행동**: R2 ≥ +0.003: label-free 전이 확인 — TGW를 설계 후보로 승격 검토(T2). R2 < +0.003이나 퇴화: TGW 계보를 종료하고 그 관측을 기록한다. 어느 경우든 SEAL 10은 열지 않는다.
- **원문 근거 (Evidence)**:
  - scripts/analysis/consensus_transfer_test.py
  - predictions/ctxq_7b/*.pt
  - scripts/analysis/tgw_transfer_test.py
  - docs/research_directions.md P4 #16
  - talks/reports/2026-09-19_trim_rate_diagnostic.md
- **선행·후속 관계 (Relations)**: 선행 RU-103(context margin 덤프) · RU-101/102(형상 브랜치 진단) · C-20260919-3(label-free 합의 가설) · research_directions P4 #16(TGW).
- **확인 필요 사항 및 한계 (Uncertainties)**: 합의 신호는 라벨을 쓰지 않으나 상관이 낮은 원인이 무정보인지 분포 이동인지 구분되지 않는다. corr 임계 0은 사전 고정한 거친 규칙이다. 신호가 퇴화하면(발동 0) 전이를 시험하지 못한 것이므로 기각이 아니라 판별 불가다.
